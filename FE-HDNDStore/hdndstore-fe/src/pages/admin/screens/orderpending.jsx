...
export default function OrderPending() {
  ...
  const handleOpenDialog = (order) => {
    ...
    setOpenDialog(true);
    setSelectedStatus("Đang giao"); // Default to "Shipping" status
  ...
  const handleUpdateStatus = async (order, newStatus, reason) => {
    ...
    try {
      await axios.put(`http://localhost:5000/orders/update/${order.idHoaDon}`, {
        ...
        ngayXacNhan: newStatus === "Đang giao" ? new Date().toISOString() : null,
      });
            ...
            <CardContent>
              <Grid container spacing={2} alignItems="center">
                <Grid item xs={12} sm={6}>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    Quản lý đơn hàng đang chờ xác nhận để giao đến khách hàng
                  </Typography>
                </Grid>
      ...
      >
        ...
        <DialogActions sx={{ borderTop: '1px solid #e0e0e0', px: 3, py: 2 }}>
          ...
          <Button
            onClick={() => {
              setViewOpen(false);
              handleOpenDialog(selectedOrderDetails);
            }}
            variant="contained"
            color="success"
            sx={{ borderRadius: 2 }}
      ...
      >
          ...
          <FormControl component="fieldset" sx={{ mb: 2 }}>
            ...
            >
              <FormControlLabel
                ...
                label={
                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <LocalShipping sx={{ mr: 1, fontSize: 20, color: theme.palette.info.main }} />
                    <Typography>Xác nhận và chuyển sang trạng thái đang giao</Typography>
                  </Box>
                }
              />
              <FormControlLabel
                ...
                label={
                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <Cancel sx={{ mr: 1, fontSize: 20, color: theme.palette.error.main }} />
        ...
        <DialogActions sx={{ borderTop: '1px solid #e0e0e0', px: 3, py: 2 }}>
          ...
          <Button
            ...
            variant="contained"
            color={selectedStatus === "Đang giao" ? "info" : "error"}
            disabled={selectedStatus === "Đã hủy" && !cancelReason}
            sx={{ borderRadius: 2 }}
          >
            {selectedStatus === "Đang giao" ? "Xác nhận đơn hàng" : "Xác nhận hủy đơn"}
          </Button>
        </DialogActions>
      </Dialog>
    </ThemeProvider>
