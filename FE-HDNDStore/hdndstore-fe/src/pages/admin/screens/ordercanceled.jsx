...
export default function OrderCanceled() {
  ...
  const filteredOrders = ordersWithUsers.filter((order) => {
    const matchesSearch =
      ...
      (order.lyDoHuy?.toLowerCase().includes(searchTerm.toLowerCase()));

    const matchesDate =
      !selectedDate ||
      dayjs(order.orderDate).format("YYYY-MM-DD") === dayjs(selectedDate).format("YYYY-MM-DD") ||
      (order.ngayXacNhan && dayjs(order.ngayXacNhan).format("YYYY-MM-DD") === dayjs(selectedDate).format("YYYY-MM-DD")) ||
      (order.ngayHuy && dayjs(order.ngayHuy).format("YYYY-MM-DD") === dayjs(selectedDate).format("YYYY-MM-DD"));
              ...
              <Box>
                ...
                <Breadcrumbs aria-label="breadcrumb" sx={{ mt: 0.5 }}>
                  ...
                  <Typography color="text.primary" sx={{ display: 'flex', alignItems: 'center' }}>
                    <Cancel sx={{ mr: 0.5 }} fontSize="small" />
                    Đơn hàng đã hủy
                  </Typography>
                </Breadcrumbs>
              </Box>
            ...
            <CardContent>
              <Grid container spacing={2} alignItems="center">
                <Grid item xs={12} sm={6}>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    Danh sách các đơn hàng đã bị hủy bởi khách hàng hoặc hệ thống
                  </Typography>
                </Grid>
                      ...
                      >
                        ...
                        <TableCell>
                          <Box sx={{ display: 'flex', alignItems: 'center' }}>
                            <CalendarToday sx={{ mr: 1, fontSize: 16, color: 'text.secondary' }} />
                            <Typography variant="body2">
                              {new Date(order.ngayHuy).toLocaleDateString('vi-VN')}
                            </Typography>
                          </Box>
                        </TableCell>
                        <TableCell>
                    ...
                    <CardContent>
                      ...
                      <Box sx={{ mt: 2 }}>
                        <Grid container spacing={2}>
                          ...
                          <Grid item xs={6}>
                            <Typography variant="body2" color="text.secondary">Ngày hủy</Typography>
                            <Typography variant="body1">
                              {new Date(selectedOrderDetails.ngayHuy).toLocaleDateString('vi-VN')}
                            </Typography>
                          </Grid>
                        </Grid>
                      </Box>
                  ...
                  <Card variant="outlined">
                    <CardContent>
                      ...
                      <Typography variant="body1" sx={{ mt: 1, color: 'error.main' }}>
                        {selectedOrderDetails.lyDoHuy || "Không có thông tin"}
                      </Typography>
